...
class App {
  constructor() {
    ...
    this.canvas.addEventListener('mousemove', this.onMouse.bind(this), false);
  }
  ...
  resize() {
    ...
    this.tmpCtx.scale(this.pixelRatio, this.pixelRatio);
  ...
  animate() {
    ...
    for(let i = 0; i < this.dots.length; i++) {
      ...
      const dis = distance(dot.x, dot.y, this.mousePos.x, this.mousePos.y);
      const half = this.pixelSize/2;
      const arg = (Math.atan(-dis/100) + Math.PI/2) / Math.PI * (half-this.radius)*2 + this.radius;
        dot.move(arg);
    }
  }
